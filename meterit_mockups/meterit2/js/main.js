...
const TEMPLATES = {
  ...
  zone: Hogan.compile(`<div class="zone-container">
                         <div id="zone-{{lane}}-{{id}}-box" class="zone left"></div>
                         <div id="zone-{{lane}}-{{id}}-brake" class="zone {{status}} right"></div>
                       </div>`)
}
...
function generateStaticBoxes() {
  ...
  lanes.forEach(lane => {
    if (lane.status == "braking") {
      for(let i = 0; i < 24; i++) {
        if (Math.random() < .25) {
          $(`#zone-${lane.id}-${i}-box`).addClass("full");
        }
      }
    } else {
      startBoxes(lane);
